...
def main():
  ...
  parser.add_argument('--model_file', help='Model path', type=str, default='../../checkpoints/fcn32s_from_caffe.pth')
  ...
  for batch_idx, (data, target) in tqdm.tqdm(enumerate(val_loader), total=len(val_loader), ncols=80, leave=False):
    if torch.cuda.is_available():
